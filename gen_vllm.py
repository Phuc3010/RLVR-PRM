...
DATA_DIR = "data"
TASKS       = [
    # {"name": "ReasonGym", "path": f"{DATA_DIR}/reasoning_gym/train.parquet", "N": 16},
    # {"name": "Polaris", "path": f"{DATA_DIR}/polaris/train.parquet", "N": 16},
    # {"name": "Polaris-Hard", "path": f"{DATA_DIR}/polaris-hard/train.parquet", "N": 16},
    # {"name": "EvoLM", "path": f"{DATA_DIR}/evolm/train.parquet", "N": 16},
    {"name": "AIME24", "path": f"eval/{DATA_DIR}/AIME24/test.parquet", "N": 1280},
    {"name": "Math-500", "path": f"{DATA_DIR}/Math-500/test.parquet", "N": 192},
    {"name": "AIME25", "path": f"{DATA_DIR}/Minerva/train.parquet", "N": 1280},
    {"name": "Olympiad-Bench", "path": f"{DATA_DIR}/Olympiad-Bench/test.parquet", "N": 192},
    {"name": "Minerva", "path": f"{DATA_DIR}/Minerva/test.parquet", "N": 192},
]
# PROMPT_TEMPLATE = """{problem}"""
PROMPT_TEMPLATE = """{problem}\nPlease reason step by step, and put your final answer within \\boxed{{}}."""
NAME        = "models/Qwen2.5-3B" # "hbx/JustRL-Nemotron-1.5B"
MAX_TOKENS  = 3072
TEMPERATURE = 0.6
TOP_P       = 0.95
OUT_DIR     = Path(f"gen_outputs/{NAME.split('/')[-1]}")
...
def load_samples(filepath: str):
    ...
    # df = df.sample(n=4096, random_state=1234).reset_index()
    if "reasoning_gym" in filepath:
        samples = [
            {
                "example_id": i,
                'data_source': df.at[i, 'data_source'],
                "prompt": df.at[i, "prompt"],
                "answer": df.at[i, "reward_model"],
            }
...
def worker_process(args_tuple):
    ...
    llm = LLM(model=NAME, enforce_eager=False, max_num_batched_tokens=16384, max_num_seqs=1024)
    results = []
    ...
    for seed in seed_list:
        ...
        if isinstance(samples[0]['prompt'], str):
            messages = [[{"role": "user", "content": s["prompt"]}] for s in samples]
        else:
            messages = [s['prompt'].tolist() for s in samples]
        outputs = llm.chat(messages, sampling, use_tqdm=True)
        for sample, out in zip(samples, outputs):
            results.append(
                {
                    "example_id": sample["example_id"],
                    "prompt": sample["prompt"].tolist() if not isinstance(sample['prompt'], str) else sample['prompt'],
